	...
	$.fn.weatherfeed = function(locations, options, fn) {	
		...
		var condition_codes = {
			...
			'fr' :{
				...
				3 :	'Orages sévères',
				4 :	'Orages',
				5 :	'Mélange de pluie et de neige',
				6 :	'Pluie et verglas',
				7 :	'Neige et verglas',
				8 :	'Bruine',
				9 :	'Bruine',
				10 :	'Pluie verglassante',
				11 :	'Averses',
				12 :	'Averses',
		...
		return this.each(function(i, e) {
			...
			var _process = function(e, feed, options) {
				...
				if (feed.description != 'Yahoo! Weather Error') {
					...
					if (options.forecast) {
						...
						for (var i=0; i<wfi.length; i++) {
							if(options.language==='en'){
								...
								html += '<div class="weatherForecastRange">'+text.high+': '+ wfi[i].high +'&deg; '+ text.low +': '+ wfi[i].low +'&deg;</div>';
								html += '</div>'
							} else {
								html += '<div class="weatherForecastItem" style="background-image: url(http://l.yimg.com/a/i/us/nws/weather/gr/'+ wfi[i].code +'s.png); background-repeat: no-repeat;">';
					...
					if (options.link) html += '<div class="weatherLink"><a href="'+ feed.link +'" target="'+ options.linktarget +'"'+ text.full_forecast_title +'">'+ text.full_forecast +'</a></div>';
